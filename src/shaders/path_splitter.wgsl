...
struct Uniform {
    ...
    camera_pos: vec3f,
    // чем больше значения тем больше зум
    camera_zoom: f32,
...
fn trace_ray_split(ro_in: vec3f, rd_in: vec3f) -> TraceData {
    ...
    var ro = ro_in;
    var rd = rd_in;
    var found_noisy = false;
    var remaining_throughput = vec3f(1.0);
    ...
    for (var i = uf.bounces; i > 0; i--) {
        ...
        if (!found_noisy) {
            ...
            if (hit_poly_idx != -1) {
                let t = abs(polygons[hit_poly_idx].t);
                // Шумящие поверхности: 0 < t < 2 (т.е. матовые и глосси)
                if (t > 0.0 && t < 2.0) {
                    is_noisy = true;
                }
            }
            ...
            if (is_noisy) {
                ...
                data.hit_mat_type = poly.t;

                remaining_throughput = vec3f(1.0);
                if (refl.is_terminal) {
                    data.reflected_light = remaining_throughput;
                    break;
                }
                ro = refl.newRo;
                rd = refl.newRd;
            ...
            if (refl.is_terminal) {
                ...
                data.reflected_light = vec3f(0.0);
                break;
            }
            ro = refl.newRo;
            rd = refl.newRd;
...
// --- ШЕЙДЕР 1: СБОР ДАННЫХ (1 СЭМПЛ) ---
@compute @workgroup_size(16, 16)
fn main_pass1(@builtin(global_invocation_id) global_id: vec3<u32>) {
